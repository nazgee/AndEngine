...
/**
 * The BoundedFifoBuffer is a very efficient implementation of Buffer that does
 * not alter the size of the buffer at runtime.
 * <p>
 ...
 * <p>
 * The {@link #add(Object)}, {@link #remove()} and {@link #get()} operations all
 * perform in constant time. All other operations perform in linear time or
 * worse.
 * <p>
 * Note that this implementation is not synchronized. The following can be used
 * to provide synchronized access to your <code>BoundedFifoBuffer</code>:
 * 
 * <pre>
 ...
 * 
 * @since 2.1
 * @version $Revision: 155406 $ $Date: 2005-02-26 12:55:26 +0000 (Sat, 26 Feb
 *          2005) $
 * 
 * @author Avalon
...
public class BoundedFifoBuffer extends AbstractCollection {
	 ...
	 */
	public BoundedFifoBufferIterator iterator() {
		return new BoundedFifoBufferIterator(this);
	}

	public static class BoundedFifoBufferIterator implements Iterator {

		private int index;
		private int lastReturnedIndex = -1;
		private boolean isFirst;
		private BoundedFifoBuffer mBuffer;

		public BoundedFifoBufferIterator(BoundedFifoBuffer pBuffer) {
			reinitialize(pBuffer);
		}

		public void reinitialize(BoundedFifoBuffer pBuffer) {
			if (pBuffer != null) {
				mBuffer = pBuffer;
				index = pBuffer.m_start;
				lastReturnedIndex = -1;
				isFirst = pBuffer.m_full;
			}
		}

		public boolean hasNext() {
			return isFirst || (index != mBuffer.m_end);
		...
		public Object next() {
			...
			index = mBuffer.increment(index);
			return mBuffer.m_elements[lastReturnedIndex];
		}
		...
		public void remove() {
			...
			if (lastReturnedIndex == mBuffer.m_start) {
				mBuffer.remove();
				lastReturnedIndex = -1;
				return;
			}
			...
			while (i != mBuffer.m_end) {
				if (i >= mBuffer.maxElements) {
					mBuffer.m_elements[i - 1] = mBuffer.m_elements[0];
					i = 0;
				} else {
					mBuffer.m_elements[i - 1] = mBuffer.m_elements[i];
					i++;
				}
			}

			lastReturnedIndex = -1;
			mBuffer.m_end = mBuffer.decrement(mBuffer.m_end);
			mBuffer.m_elements[mBuffer.m_end] = null;
			mBuffer.m_full = false;
			index = mBuffer.decrement(index);
		}
